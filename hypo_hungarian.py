...
class Hypo_Auctioneer():
    def __init__(self):
        ...
        self.hypo_tasks = []
        self.hypo_prob = []
        self.num_of_hypo_tasks =2
        self.bid_robot_task_array = np.zeros((self.num_robots, self.num_of_hypo_tasks))
    ...
    def check_hypo_bids(self,msg):  # update the finish_cond for checking whether robots have done the tasks or not.
        self.bid_robot_task_array[int(msg.robot_id[6]), self.hypo_tasks.index(msg.task)] = msg.bid
        self.num_bids += 1
    ...
    def auction_step(self):
        row_ind, col_ind = linear_sum_assignment(self.bid_robot_task_array)
        for i in range(len(row_ind)):
            self.robot_id = "robot_" + str(i)
            self.task = self.hypo_tasks[row_ind[i]]
